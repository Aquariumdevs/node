...
type App struct {
	...
	txStorageTree  *MerkleTree
	txStorageTree2 *MerkleTree
	blockHashTree  *MerkleTree
	validatorTree  *arbo.Tree

	computePoseidon *MerkleTree
...
func NewApp() (*App, error) {
	...
	_, app.computePoseidon, _ = app.createPoseidonTreeDb("n00ully", 64, true)

	// create 2 temporary swaping Trees of transactions
	txStorageDb, txStorageTree, err := app.createPoseidonTreeDb(badg2, 64, true)
	if err != nil {
		logs.logError("Tree txStorageTree initialization failed!!!", err)
		return nil, err
	}
	txStorageDb2, txStorageTree2, err := app.createPoseidonTreeDb(badg3, 64, true)
	if err != nil {
		logs.logError("Tree txStorageTree initialization failed!!!", err)
		return nil, err
	}
	...
	blockHashDb, blockHashTree, err := app.createPoseidonTreeDb("badg4", 64, true)
	if err != nil {
...
func (app *App) DeliverTx(req abcitypes.RequestDeliverTx) abcitypes.ResponseDeliverTx {
	...
	if tx.isUpdate {
		logs.log("	update")
		tx.calcPoseidon(app, tx.source, tx.state)
		tx.execUpdate(app)
	}

	if tx.isTransfer {
		logs.log("	transfer")
		tx.calcPoseidon(app, tx.source, tx.target, tx.amount, tx.state)
		tx.execTransfer(app)
	}
	...
	if tx.isAccountCreator {
		logs.log("	create")
		tx.calcPoseidon(app, tx.source, tx.pubkey, tx.state)
	...
	app.txDbVals = append(app.txDbVals, tx.poseidon[:])
...
func (app *App) Commit() abcitypes.ResponseCommit {
	...
	//TODO: write the blockRoot to the txDbVals with key 00000000 (append, see example above)
	...
	resp := app.calcPoseidon(chainroot, resthash, blockRoot)
...
func (app *App) Query(reqQuery abcitypes.RequestQuery) (resQuery abcitypes.ResponseQuery) {
	...
	case 8:
		...
		app.txStorageTree2.RecoverTree()
		//TODO:add stuff manually to check if problem is on addbatch or genproof
		_, _, path1, err := app.txStorageTree2.GenProof(key)
		blockroot, _ := app.txStorageTree2.Root()
		if err != nil {
			app.txStorageTree.RecoverTree()
			_, _, path1, err = app.txStorageTree.GenProof(key)
			blockroot, _ = app.txStorageTree.Root()
		}

		height := make([]byte, 8)
		copy(height, app.blockheight[:])

		app.blockHashTree.RecoverTree()

		_, _, path2, err2 := app.blockHashTree.GenProof(height) ////TODO:diff blockheight for tree2
		if err2 != nil {
			height[4]--                                         //TODO: bigendian
			_, _, path2, _ = app.blockHashTree.GenProof(height) ////TODO:diff blockheight for tree2

		}
		app.txDbMutex.Unlock()

		value = append(key, blockroot...)
		value = append(value, blockroot...)
		value = append(value, path1...)

		chainroot, _ := app.blockHashTree.Root()
		value = append(value, height...)
		value = append(value, chainroot...)
		value = append(value, chainroot...)
		value = append(value, path2...)
