...
fn build_full_merkle_tree(data: Vec<Fp>) -> MerkleNode {
  LOCAL_TREE.with(|levels| {
    ...
    while levels.last().unwrap().len() > 1 {
        ...
        // Access the root without modifying the tree structure
        if let Some(last_level) = levels.last() {
            if let Some(root) = last_level.first() {
                return MerkleNode {
                    hash: root.hash,
                    left: root.left,
                    right: root.right,
        ...
        // Fallback in case of an unexpected empty level
        MerkleNode {
            hash: Fp::zero(),
            left: std::ptr::null_mut(),
            right: std::ptr::null_mut(),
...
pub fn get_merkle_path_impl(leaf_index: usize) -> Vec<Fp> {
    ...
    LOCAL_TREE.with(|tree| {
        ...
        for level in 0..tree.len() - 1 {
            ...
            if let (Some(current_node), Some(sibling_node)) = (nodes.get(current_index), nodes.get(sibling_index)) {
                let current_hash = unsafe { (*current_node).hash };
                let sibling_hash = unsafe { (*sibling_node).hash };
                
                if current_hash != sibling_hash {
                    // Push both nodes if they are not identical
                ...
                // If hashes are identical, omit both nodes
            }
...
fn get_root() -> Fp {
    LOCAL_TREE.with(|levels| {
        ...
        if levels.is_empty() {
            Fp::zero() // Return zero if the tree is empty
        } else {
            let root_level = levels.last().unwrap(); // Access the last level
            if root_level.is_empty() {
                Fp::zero() // If the root level is empty, return zero
            } else {
                root_level.first().unwrap().hash // Return the hash of the first (root) node
            }
        }
    })
}
